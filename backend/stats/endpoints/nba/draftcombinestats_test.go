...
func TestDraftCombineStats(t *testing.T) {
	validLeagueID := "00"
	validSeasonYear := "2019-20"
	validSeasonAllTime := "All Time"
	invalidLeagueID := "99"
	invalidSeasonYear := "19-20"
	...
	tests := []struct {
		...
		{validLeagueID, validSeasonAllTime, false}, // Valid "All Time"
		{invalidLeagueID, validSeasonYear, true},   // Invalid LeagueID
		{validLeagueID, invalidSeasonYear, true},   // Invalid SeasonYear format
		{"", validSeasonYear, true},                // Missing LeagueID
		{validLeagueID, "", true},                  // Missing SeasonYear
	}

	for _, test := range tests {
		resp, err := DraftCombineStats(test.leagueID, test.seasonYear)
