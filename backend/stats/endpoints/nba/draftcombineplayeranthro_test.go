...
// TestDraftCombinePlayerAnthro validates correct and incorrect parameters.
func TestDraftCombinePlayerAnthro(t *testing.T) {
	tests := []struct {
		leagueID   string
		seasonYear string
		expectErr  bool
	...
	for _, test := range tests {
		_, err := DraftCombinePlayerAnthro(test.leagueID, test.seasonYear)
		if test.expectErr {
			if err == nil {
