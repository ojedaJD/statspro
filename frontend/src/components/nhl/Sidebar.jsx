...
const Sidebar = ({ oddsData, onPlayerSelect }) => {
    ...
    const getPropTypeLabel = () => {
        ...
        return propTypeMap[sidebarFilters.propType];
    };
                            ...
                            <Box sx={{ width: '100%' }}>
                                <Box sx={{ display: 'flex', justifyContent: 'space-between', width: '100%', mb: 0.5 }}>
                                    ...
                                    <Typography variant="caption" color="text.secondary">
                                        {team.Abbreviation} - {player.odds?  'Active' : 'Inactive'}
                                    </Typography>
                                </Box>
    ...
    const renderGames = () => {
        return filteredOddsData.map((game, index) => {
            const homeTeamSection = renderTeamSection(game.Home);
            const awayTeamSection = renderTeamSection(game.Away);

            if (!homeTeamSection && !awayTeamSection) return null;

            const gameTitle = `${game.Away.Abbreviation} @ ${game.Home.Abbreviation}`;
